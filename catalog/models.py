...
class BorrowRequest(models.Model):
    ...
    def clean(self):
        if not self.pk:
            if self.status not in [self.Status.PENDING, self.Status.APPROVED, self.Status.REJECTED]:
                 raise ValidationError("New requests can only be Pending, Approved, or Rejected.")
        ...
        if self.pk:
            ...
            if old_instance.status == self.Status.RETURNED:
                raise ValidationError("Cannot edit a request that has already been returned.")

            # If status was APPROVED, only allow transition to RETURNED, LOST, or OVERDUE
            if old_instance.status == self.Status.APPROVED:
                if self.status not in [self.Status.APPROVED, self.Status.RETURNED, self.Status.LOST, self.Status.OVERDUE]:
                     raise ValidationError("Approved requests can only be changed to Returned, Lost, or Overdue.")
            ...
            if self.pk:
                ...
                if old_instance.status != self.Status.APPROVED:
                     if self.book_item.status != BookItem.Status.AVAILABLE:
                         raise ValidationError(f"Book item {self.book_item.barcode} is not available (Status: {self.book_item.get_status_display()}).")
            else:
                 if self.book_item and self.book_item.status != BookItem.Status.AVAILABLE:
                     raise ValidationError(f"Book item {self.book_item.barcode} is not available (Status: {self.book_item.get_status_display()}).")

        if self.status == self.Status.RETURNED:
             if self.pk:
                old_instance = BorrowRequest.objects.get(pk=self.pk)
                if old_instance.status not in [self.Status.APPROVED, self.Status.OVERDUE, self.Status.LOST]:
                    raise ValidationError("Only approved, overdue, or lost requests can be returned.")
    ...
    def save(self, *args, **kwargs):
        ...
        if self.status == self.Status.APPROVED and (not old_status or old_status != self.Status.APPROVED):
        ...
        if self.book_item:
            ...
            elif self.status == self.Status.RETURNED and old_status != self.Status.RETURNED:
