...
def export_categories_excel(request):
    ...
    wb = render_categories_workbook(qs, columns=columns, include_books=include_books)
...
def export_books_excel(request):
    ...
    wb = render_books_workbook(qs, columns=columns, include_items=include_items)
...
def export_publishers_excel(request):
    ...
    wb = render_publishers_workbook(qs, columns=columns, include_books=include_books)
...
def author_stats_api(request):
    ...
    deceased_authors = Author.objects.filter(death_date__isnull=False).order_by(
        "death_date"
    )
...
def export_authors_excel(request):
    ...
    wb = render_authors_workbook(qs, columns=columns, include_books=include_books)
