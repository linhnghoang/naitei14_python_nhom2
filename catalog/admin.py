    ...
    def save_model(self, request, obj, form, change):
        ...
        if obj.isbn13:
            ...
            if len(isbn) != 13 or not isbn.isdigit():
                ...
                messages.warning(
                    request, 
                    "ISBN-13 should be 13 digits. Please verify the format."
                )
        ...
        if obj.publish_year and obj.publish_year > current_year + 1:
            ...
            messages.warning(
                request, 
                f"Publish year {obj.publish_year} is in the future. Please verify this is correct."
            )
        ...
        if not change:  # New book
            ...
            messages.success(
                request,
                f"Book '{obj.title}' has been created successfully. You can now add authors and categories using the sections below."
            )
...
class BookAuthorAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Order", {
            "fields": ("author_order",),
            "description": "Specify the order of this author for the book (for multiple authors)."
        }),
    )
...
class BookItemAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if obj.barcode:
            ...
            if existing.exists():
                ...
                messages.error(
                    request,
                    f"Barcode '{obj.barcode}' already exists. Please use a unique barcode."
                )
                return
        ...
        if not change:  # New item
            ...
            messages.success(
                request,
                f"Book item with barcode '{obj.barcode}' has been created successfully."
            )
    ...
    def mark_as_available(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully marked {count} items as available.",
            level='success'
        )
    ...
    def mark_as_lost(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully marked {count} items as lost.",
            level='warning'
        )
    ...
    def mark_as_damaged(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully marked {count} items as damaged.",
            level='warning'
        )
