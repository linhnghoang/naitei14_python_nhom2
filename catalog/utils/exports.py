...
DEFAULT_CATEGORY_COLUMNS = [
    ...
    'books_count', 'children_count', 'hierarchy_level'
]
...
def build_category_queryset(params, include_books=False):
    ...
    if 'has_children' in params and params['has_children'].lower() in ['true', '1', 'yes']:
        qs = qs.filter(children__isnull=False).distinct()
    elif 'has_children' in params and params['has_children'].lower() in ['false', '0', 'no']:
...
def render_categories_workbook(queryset, columns=None, include_books=False):
    ...
    if include_books:
        ...
        # Books sheet headers
        books_headers = ['Category ID', 'Category Name', 'Category Path', 'Book ID', 'Book Title', 'ISBN13', 'Publisher', 'Year']
...
def build_book_queryset(params, include_items=False):
    """Build book queryset based on filter parameters - for compatibility with base project."""
    qs = Book.objects.all()
...
def render_books_workbook(queryset, columns=None, include_items=False):
    """Create an Excel workbook with books data - for compatibility with base project."""
    wb = Workbook()
    ws = wb.active
    ws.title = "Books"
    ...
    for row_idx, book in enumerate(queryset, 2):
        ...
        ws.cell(row=row_idx, column=4).value = book.publisher.name if book.publisher else ''
        ws.cell(row=row_idx, column=5).value = book.publish_year or ''
        ws.cell(row=row_idx, column=6).value = ', '.join([cat.name for cat in book.categories.all()])
        ws.cell(row=row_idx, column=7).value = ', '.join([auth.name for auth in book.authors.all()])
...
def build_publisher_queryset(params, include_books=False):
    ...
    if 'has_website' in params and params['has_website'].lower() in ['true', '1', 'yes']:
        qs = qs.exclude(Q(website='') | Q(website__isnull=True))
    elif 'has_website' in params and params['has_website'].lower() in ['false', '0', 'no']:
...
def render_publishers_workbook(queryset, columns=None, include_books=False):
    ...
    for publisher in queryset:
        ...
        for col_idx, col_name in enumerate(columns, 1):
            ...
            if col_name == 'id':
                ...
                cell.value = publisher.created_at.strftime('%Y-%m-%d %H:%M:%S') if publisher.created_at else ''
    ...
    if include_books:
        ...
        # Books sheet headers
        books_headers = ['Publisher ID', 'Publisher Name', 'Founded Year', 'Book ID', 'Book Title', 'ISBN13', 'Year Published', 'Pages', 'Language']
        for col_idx, header in enumerate(books_headers, 1):
            cell = books_ws.cell(row=1, column=col_idx)
            cell.value = header
            cell.font = header_font
        ...
        for publisher in queryset:
            ...
            if books:
                for book in books:
                    ...
                    books_ws.cell(row=books_row, column=5).value = book.title
                    books_ws.cell(row=books_row, column=6).value = book.isbn13 or ''
                    books_ws.cell(row=books_row, column=7).value = book.publish_year or ''
                    books_ws.cell(row=books_row, column=8).value = book.pages or ''
                    books_ws.cell(row=books_row, column=9).value = book.language_code or ''
                    books_row += 1
            else:
